...
Download the ZITADEL release according to your architecture from [Github](https://github.com/dennigogo/zitadel/releases/latest)
...
LATEST=$(curl -i https://github.com/dennigogo/zitadel/releases/latest | grep location: | cut -d '/' -f 8 | tr -d '\r'); wget -qO-  https://github.com/dennigogo/zitadel/releases/download/$LATEST/zitadel_Darwin_$(uname -m).tar.gz  | tar -xJ zitadel && sudo mv zitadel /usr/local/bin
```
